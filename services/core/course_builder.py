...
class SkeletonBuilder:
    ...
    def _build_inner(
        ...
        prompt = (
            ...
            + (f"\n{_syllabus_evidence_block}\n\n"
               "USE THE CHAPTER LISTS ABOVE AS YOUR SOURCE OF TRUTH FOR WHAT THIS "
               "SUBJECT CONTAINS. They are real syllabi from open textbooks. "
               "Your job is to SELECT and SEQUENCE from them to fit the "
               "requested scope and level — not to invent a different syllabus, "
               "and not to copy them wholesale. If the lists contain a topic "
               "central to the subject, it belongs in this course. If they "
               "contain far more than the requested scope allows, choose the "
               "load-bearing topics and say so in the rationale.\n\n"
    ...
    def _syllabus_evidence(self, topic):
        """Real chapter lists for this subject, to inform the skeleton.

        THE WEAKNESS THIS ADDRESSES. The skeleton was generated by the LLM
        alone: "give me modules for X". Nothing checked the result against how
        the subject is actually taught, and the measured consequence was a
        course covering 42% of its own subject while passing every structural
        detector we run.

        An LLM asked to enumerate a syllabus produces what it can recall of
        one. A textbook's chapter list IS a syllabus, written by someone who
        teaches the subject. Supplying it turns the model from an AUTHOR of
        imagined structure into an EDITOR of real structure.

        Returns "" when nothing is found — generation then proceeds unguided,
        which is the old behaviour, and the caller logs that it is degraded
        rather than pretending it was informed.
        """
        if os.getenv("HELGA_SYLLABUS_EVIDENCE", "1") == "0":
            return ""
        try:
            from services.research.syllabus_sources import (
                subject_outline, format_for_prompt)
        except Exception as e:
            logger.info(f"[SKELETON] syllabus evidence unavailable: {e}")
            return ""

        # A narrow topic has no book of its own — "the pythagorean theorem"
        # matches no Wikibook — so ask the model for the parent discipline and
        # search that too. One cheap call buys a real syllabus.
        broader = []
        try:
            raw = llm_generate(
        ...
        try:
            outline = subject_outline(topic, broader_subjects=broader)
        except Exception as e:
            logger.warning(f"[SKELETON] subject_outline failed: {e}")
            return ""

        if not outline.get("found"):
            logger.warning(
                f"[SKELETON] no open-textbook syllabus for {topic!r} "
        ...
        books = ", ".join(f"{o['source']}:{o['book']}" for o in outline["outlines"])
        n = sum(len(o["chapters"]) for o in outline["outlines"])
        logger.info(f"[SKELETON] syllabus evidence for {topic!r}: {n} chapters from {books}")
        if self.status_callback:
            self.status_callback(f"CHECK:SYLLABUS_EVIDENCE:{n} chapters")
        return format_for_prompt(outline)
