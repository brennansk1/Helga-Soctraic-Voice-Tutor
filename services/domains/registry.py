...
def domain_for(subject, llm_json_fn=None):
    ...
    for key in available():
        ...
        for w in (getattr(mod, "KEYWORDS", ()) or ()):
            ...
            # "api" must not fire inside "therapist", but "computer science"
            # should fire inside "intro to computer science".
            if len(w) <= 4:
                import re as _re
                if _re.search(rf"\b{_re.escape(w)}\b", s):
                    return key
            elif w in s:
                return key
    if llm_json_fn:
        return _classify_with_llm(s, llm_json_fn)
    return None


def _classify_with_llm(subject, llm_json_fn):
    """Ask the model which domain a subject belongs to, or None.

    Returns None rather than guessing when the model is unsure — a subject
    routed to the wrong domain gets actively wrong teaching guidance, which is
    ...
    prompt = (
        ...
        f"Answer 'none' unless the subject clearly belongs to one. A wrong "
        f"domain gives the tutor actively wrong teaching instructions.\n\n"
...
def for_subject(subject, llm_json_fn=None):
