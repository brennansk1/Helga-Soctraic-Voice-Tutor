...
async function startCustomGeneration() {
    ...
    try {
        ...
        // The endpoint ACKs the build and returns; it does not wait for it.
        // `course_uid: "pending"` is that ACK — treating it as a finished
        // course showed the completion panel seconds in and wired the Start
        // button to /learn?course_uid=pending. The real uid arrives with the
        // DONE stage, minutes later.
        if (result.course_uid && result.course_uid !== 'pending') {
            showWizardComplete(result.course_uid);
        } else if (result.course_uid === 'pending' || result.status === 'building') {
            statusEl.textContent = 'Building — this takes a while. You can watch the structure appear below.';
        } else {
