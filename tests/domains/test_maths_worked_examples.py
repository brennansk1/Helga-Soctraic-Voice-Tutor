...
def test_an_aided_kind_gets_a_teaching_move():
    course = _course(["PROCEDURE"])
    tally = we.attach_to_course(course, BOOK)
    move = _concepts(course)[0].get("teaching_move")
...
def test_a_definition_gets_nothing():
    ...
    assert _concepts(course)[0].get("teaching_move") is None
...
def test_the_same_example_is_not_attached_twice():
    ...
    we.attach_to_course(course, BOOK)
    attached = [c["teaching_move"] for c in _concepts(course)
                if c.get("teaching_move")]
