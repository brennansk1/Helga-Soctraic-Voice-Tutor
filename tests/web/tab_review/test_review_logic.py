...
class TestReviewTab(unittest.TestCase):
    ...
    def test_get_due_cards(self):
        ...
        with patch('app.requests.get') as mock_get:
            ...
            mock_request.args = {'topic': 'Math', 'course_uid': 'c1'}
            ...
            mock_resp.json.return_value = [{"front": "Q", "back": "A"}]
            mock_get.return_value = mock_resp
            
            from app import get_due_cards
            resp, code = get_due_cards()
            
            self.assertEqual(code, 200)
            self.assertEqual(resp, [{"front": "Q", "back": "A"}])

    def test_update_card(self):
        import app
        with patch('app.requests.post') as mock_post:
            mock_request = MagicMock()
            mock_request.json = {"card_id": 1, "grade": "good"}
            app.request = mock_request
            app.jsonify = lambda x: x
            ...
            from app import update_card
            resp, code = update_card()
