...
def run_generation():
    ...
    builder = SkeletonBuilder(db_path=DB_PATH, course_depth=3) # Default depth
    ...
    # Initialize Hydrator (mocking providers for now, just relies on LLM fallback/generation mostly)
    # real ecosystem might have ZimProvider available, assume it might not be perfect in test env
    try:
